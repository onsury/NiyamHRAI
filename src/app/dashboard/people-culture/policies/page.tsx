              ...
              {CATS.map(c => {
                ...
                return (
                  <button key={c.id} onClick={() => setActiveCat(c.id)} className={`w-full text-left px-4 py-3 flex items-center gap-3 transition-all ${activeCat === c.id ? 'bg-amber-50' : 'hover:bg-slate-50'}`}>
                    ...
                    {has ? <span className="w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center text-white text-[10px] font-bold">âœ“</span> : <span className="w-5 h-5 bg-slate-200 rounded-full" />}
                  </button>
                );
              })}
            </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border border-slate-200 rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-4">
              <button onClick={savePractices} disabled={saving} className="flex-1 py-3.5 bg-slate-900 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-800 disabled:opacity-30">{saving ? 'Saving...' : saved ? 'âœ“ Saved!' : 'Save Progress'}</button>
              {filledCount >= 3 && <button onClick={runAnalysis} disabled={analyzing} className="flex-1 py-3.5 bg-gradient-to-r from-amber-500 to-orange-600 text-black rounded-xl font-black text-xs uppercase tracking-widest disabled:opacity-30">{analyzing ? 'Analysing...' : 'ðŸ§  Run AI Analysis'}</button>}
            </div>
          </div>
